...
${!eventData.isPR ? `- Never push directly to "dev" or "master" branches. Always use the example below to create a pull request` : ""}
${
  useCommitSigning
...
${!eventData.isPR ? `- Clone target repository: Bash(git clone --branch ${process.env.TARGET_REPO_BASE_BRANCH} --single-branch https://x-access-token:${process.env.GITHUB_TOKEN}/${process.env.GH_HOST}/<owner>/<repo_name> ${process.env.RUNNER_TEMP}/<repo_name>_<timestamp>) (ALWAYS use --branch and --single-branch with specified target branch)` : ""}
  - Stage files: Bash(git add <files>)
...
${!eventData.isPR ? `- Create Pull Request: Bash(gh pr create --draft --label "<comma_seperated_issue_labels>" --repo <owner>/<repo_name> --title "<title>" --body "<desciptive_commit_message>" --base ${process.env.TARGET_REPO_BASE_BRANCH}) (It is CRITICAL to use --base with branch "${process.env.TARGET_REPO_BASE_BRANCH}")` : ""}
${!eventData.isPR ? `- Add labels to Pull Request: Bash(gh pr edit <PR-number> --add-label "<label-1>" --add-label "<label-n>") (It is Critical to run this command, to ensure issue labels are always reflect pull requests)` : ""}
